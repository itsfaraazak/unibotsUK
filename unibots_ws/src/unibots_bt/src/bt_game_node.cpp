...
private:
    ...
    // SCOOP: fire claw once per capture entry.
    bool         scoop_fired_{false};
    ...
    bool ball_in_blindspot()
    {
        ...
        scoop_fired_ = false;   // ball left blind spot — reset for next entry
        return false;
    }
    ...
    {
        set_state("CAPTURE");
        // Fire claw once per capture attempt (ball is in blind spot → close range).
        if (!scoop_fired_) {
            publish_servo("SCOOP");
            scoop_fired_ = true;
        }
        ...
        ++balls_held_;
        scoop_fired_ = false;   // reset for next capture
